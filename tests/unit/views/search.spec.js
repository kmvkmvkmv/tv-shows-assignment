...
describe('In Search View', () => {
    ...
    let getters = {
        getSearchQuery: () => 'some query',
        getSearchResult: () => data
    }
    let actions = {
        changeSearchResult: jest.fn()
    }
    ...
    it('testing watcher with some query', () => {
        wrapper.vm.$options.watch.searchQuery.call(wrapper.vm, 'something');
    })

    it('testing with null value of searchquery', () => {
        getters.getSearchQuery = () => null
        let store = new Vuex.Store({
            getters
        })
        wrapper = shallowMount(Search, {
